...
Yao et al. define and measure sycophancy between debating agents, where an agent copies a peer answer instead of testing it, and they leave correction to future work~\cite{yao2025peacemaker}. Sharma et al. trace the behavior to training, because models learn to agree with plausible positions during preference tuning~\cite{sharma2024sycophancy}. A survey of the field lists workflow patterns and open challenges for multi-agent systems and names reliability under social pressure as an unresolved item~\cite{li2024survey}. The field also reaches applied settings, such as debate with external knowledge, adaptive debate for educational reasoning, debate for research ideation, debate for scientific code generation, and debate in communications engineering~\cite{wang2025learning,zhou2025adaptive,ueda2025exploring,wang2025scalability,gomez2026multiagent}. Knowledge integration supplies the external signal that debate lacks, and a survey of LLM and knowledge-base methods reports factual gains from grounded retrieval~\cite{yang2025comprehensive}.

The mitigation lines that exist each leave part of the gap open. Prompt refinement rewrites the debate prompt between rounds, and the final decision still rests on self-reported confidence~\cite{pitre-etal-2025-consensagent}. Uncertainty weighting reweights agents from token-level signals, and its own ground-truth probe shows that the gain is bounded by the quality of that internal signal~\cite{yoffe-etal-2025-debunc}. Confidence-weighted consensus pools diverse models with self-estimated confidence, which is the same weak channel~\cite{chen2024reconcile}. A post-hoc classifier flips a suppressed majority after the debate ends, once the pressure on the minority has already been applied~\cite{he2026minority}. Diversity and training-time methods change how agents reason or how they are adapted, not how much each agent counts in the final vote~\cite{liu2025breaking,subramaniam2025multiagent}. Intervention testing reports that most fixes for sycophancy are not sufficient on their own~\cite{wynn2025talk}. Efficiency work lowers the token cost of a debate and leaves the aggregation step unchanged~\cite{fan2026imad,li-etal-2024-improving-multi}. Across these lines, no method verifies an agent's claims against external evidence while the debate runs, and no method uses that verification to set the weight of the agent in the final answer.
