...
This section defines the datasets, the baselines, the metrics, the injection protocol, the serving setup, and the statistical tests that make up the evaluation. Table~\ref{tbl:datasets} lists the question sets, and Table~\ref{tbl:baselines} lists the baseline conditions.
...
The evaluation compares the trust-calibrated system with nine baseline conditions. Table~\ref{tbl:baselines} lists the conditions. The set covers single-agent answering, debate with and without retrieval, self-consistency sampling, static aggregation, an API oracle, and the two closest published competitors. Baseline B1 answers with one chain-of-thought pass~\cite{wei2022chain}. Baseline B3 is the vanilla debate loop of Du et al.~\cite{du2024improving}, and the same loop is the Gate 0 reproduction target. Baseline B5 samples several reasoning paths and keeps the most consistent answer~\cite{wang2023selfconsistency}. Baseline B6 is the layered mixture-of-agents pipeline~\cite{wang2025mixture}. Baseline B7 sends each question to a large commercial model and records the accuracy ceiling. Baseline B9 is the iMAD reimplementation, and baseline B10 is the ConsensAgent comparison~\cite{fan2026imad,pitre-etal-2025-consensagent}. The retrieval conditions B2 and B4 use the same source-partitioned retriever as the main system. The only difference between B1 and B2, or between B3 and B4, is the evidence channel, which isolates the effect of retrieval from the effect of debate.

\begin{table}
\caption{Baseline conditions in the evaluation.}\label{tbl:baselines}
\renewcommand{\arraystretch}{1.2}
\begin{tabularx}{\textwidth}{|c|p{5.4cm}|X|}
\hline
\textbf{ID} & \textbf{Condition} & \textbf{Reference} \\
\hline
B1 & Single-agent chain of thought & Wei et al.~\cite{wei2022chain} \\
\hline
B2 & Single-agent with retrieval & This work, retrieval channel \\
\hline
B3 & Vanilla multi-agent debate & Du et al.~\cite{du2024improving} \\
\hline
B4 & Multi-agent debate with retrieval & This work, retrieval channel \\
\hline
B5 & Self-consistency sampling & Wang et al.~\cite{wang2023selfconsistency} \\
\hline
B6 & Mixture-of-agents aggregation & Wang et al.~\cite{wang2025mixture} \\
\hline
B7 & Oracle model ceiling & Commercial API model \\
\hline
B8 & Trust-calibrated deliberation & This work \\
\hline
B9 & iMAD reimplementation & Fan et al.~\cite{fan2026imad} \\
\hline
B10 & ConsensAgent comparison & Pitre et al.~\cite{pitre-etal-2025-consensagent} \\
\hline
\end{tabularx}
\end{table}
