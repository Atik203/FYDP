...
Large language models can answer scientific questions at a level that makes them useful for tutoring and decision support. A model can still be confidently wrong in fluent language that hides the error. Multi-agent debate reduces this risk because three agents answer the same question and review each other's reasoning over rounds~\cite{du2024improving}. Chain-of-thought prompting exposes the steps behind each answer, so peers can inspect the reasoning~\cite{wei2022chain}. The weak point sits at the end of the debate. Most systems combine the answers by majority vote, where every agent counts the same and the most frequent answer wins~\cite{wang-etal-2024-rethinking-bounds}. A confident and wrong majority can then push a correct minority agent to drop its answer, and the vote records the wrong result as agreement~\cite{choi2025debate}. This failure is called sycophantic consensus~\cite{pitre-etal-2025-consensagent}. Yao et al. define and measure the inter-agent form, where an agent copies a peer answer instead of testing it, and they leave correction to future work~\cite{yao2025peacemaker}. Sharma et al. trace the tendency to preference training, which teaches models to agree with plausible positions~\cite{sharma2024sycophancy}. No current debate system checks an agent's claims against outside scientific evidence before that agent earns influence. This study builds a trust-calibrated multi-agent deliberation framework to close that gap. Each agent's answer is split into small factual claims. The claims are checked against retrieved scientific literature, and every agent holds a bounded trust score that rises with supporting evidence and falls with contradicting evidence. The final answer comes from trust-weighted aggregation instead of a head count. The method is tested on GPQA and MMLU-Pro, and it needs no fine-tuning because all models run as served inference.

Du et al. show that debate improves factuality and reasoning over a single pass, but their agents are treated equally and their claims are never checked against external sources~\cite{du2024improving}. Pitre et al. report that the correct answer is present in the discussion yet ignored in more than 20 percent of wrong-answer cases, and their prompt-rewriting fix still leaves the final decision on self-reported confidence~\cite{pitre-etal-2025-consensagent}. Yoffe et al. reweight agents from token-level signals, and their own ground-truth experiment caps the gain by the quality of that signal~\cite{yoffe-etal-2025-debunc}. ReConcile pools diverse models and weights them by self-estimated confidence, the same internal channel~\cite{chen2024reconcile}. Token cost is the target of Fan et al., who decide when a debate is worth running, yet majority voting remains once it starts~\cite{fan2026imad}, and sparse topologies cut communication cost without touching the aggregation~\cite{li-etal-2024-improving-multi}. Wang et al. combine many models through static aggregation and score well on open benchmarks, with no per-agent trust and no evidence check~\cite{wang2025mixture}. Diversity and training-time methods change how agents reason, not how much each agent counts in the vote~\cite{liu2025breaking,subramaniam2025multiagent}, and intervention testing shows most sycophancy fixes are not sufficient on their own~\cite{wynn2025talk}. He et al. detect a suppressed correct minority after the debate and overturn the majority, which comes too late to protect the minority while it argues~\cite{he2026minority}.

Across these systems, influence is set by a vote or an internal score, and none of them checks an agent's claims against evidence while the debate runs. Self-reported confidence is a weak basis, because training that makes models agreeable also makes them more sycophantic~\cite{ibrahim2026training}, and alignment work shows agreement with wrong user positions rises after tuning~\cite{chen2025self}. Theory predicts that correlated agents drift to a wrong majority unless an outside correction enters the loop~\cite{estornell2024multillm}, and multi-agent systems fail in recurring ways that diagnosis alone does not fix~\cite{cemri2025why}. Surveys map the design space and its open challenges~\cite{li2024survey}. Applied variants adapt debate to knowledge-grounded reasoning~\cite{wang2025learning} and scientific code generation~\cite{wang2025scalability}, while others study research ideation~\cite{ueda2025exploring}, educational reasoning~\cite{zhou2025adaptive}, and communications engineering~\cite{gomez2026multiagent}. This study ties each agent's influence to external evidence during the debate. Claims are checked against retrieved passages from scientific sources~\cite{karpukhin2020dense}, in line with work that grounds correction in external tools~\cite{gou2024critic} and with studies that report factual gains from knowledge grounding~\cite{yang2025comprehensive}. The trust score is bounded, so no agent is silenced or made all-powerful, and the final decision uses those weights. Scientific question sets are a suitable testbed because they carry verifiable answers and published literature that retrieval can reach, and GPQA and MMLU-Pro allow comparison against established baselines. The expected outcome is a reproducible framework that lowers consensus collapse, preserves a correct minority more often, keeps answer accuracy stable, and reports a calibrated trust signal.
...
This section defines the datasets, the baselines, the metrics, the injection protocol, the serving setup, and the statistical tests that make up the evaluation. Table~\ref{tbl:datasets} lists the question sets.

\begin{center}
\captionof{table}{Datasets used in the evaluation. BrokenMath and BrokenArXiv create a false majority, HLE sets the difficulty ceiling, and GPQA and MMLU-Pro measure accuracy under stable conditions.}\label{tbl:datasets}
\renewcommand{\arraystretch}{1.2}
\begin{tabularx}{\textwidth}{|p{2.1cm}|p{2.4cm}|X|p{2.2cm}|p{2.3cm}|}
\hline
\textbf{Dataset} & \textbf{Role} & \textbf{Source} & \textbf{License} & \textbf{Size} \\
\hline
BrokenMath & Adversarial & INSAIT-Institute, Hugging Face~\cite{petrov2025brokenmath} & CC BY-NC-SA 4.0 & 451 problems \\
\hline
BrokenArXiv & Adversarial & MathArena, Hugging Face~\cite{dekoninck2026matharena} & CC BY-SA 4.0 & 87 problems \\
\hline
HLE & Difficulty ceiling & Center for AI Safety, Hugging Face~\cite{phan2026hle} & MIT, gated & 2{,}500 questions \\
\hline
GPQA Diamond & Stable baseline & Idavidrein, Hugging Face~\cite{rein2024gpqa} & CC BY 4.0, gated & 198 questions \\
\hline
MMLU-Pro & Stable baseline & TIGER-Lab, Hugging Face~\cite{wang2024mmlupro} & MIT & Over 12{,}000 questions \\
\hline
\end{tabularx}
\end{center}
...
The evaluation uses five public scientific question sets. The sets fall into three roles. Two adversarial sets create a false majority on purpose, one set holds expert-level questions that act as a difficulty ceiling, and two sets measure accuracy without adversarial pressure. Table~\ref{tbl:datasets} lists the role, the source, the license, and the official size of each set.

The adversarial pair is BrokenMath~\cite{petrov2025brokenmath} and BrokenArXiv~\cite{dekoninck2026matharena}. BrokenMath is built from 2025 competition problems that a language model perturbs into false statements, and human experts then review the perturbed versions. The release contains 451 benchmark problems written as proof tasks. BrokenArXiv turns false statements from recent arXiv papers into problems and publishes new snapshots each month, and the 2026 release used here contains 87 problems. Both sets place an agent in front of a wrong premise that reads as plausible, which is the condition that drives sycophantic agreement in a debate.

Humanity's Last Exam~\cite{phan2026hle} sets the ceiling. The set contains 2{,}500 expert-written questions across dozens of subject areas, and the release is gated behind a free click-through agreement with a no-redistribution rule. GPQA~\cite{rein2024gpqa} and MMLU-Pro~\cite{wang2024mmlupro} give the stable comparison. GPQA contains 448 graduate-level multiple-choice questions in biology, physics, and chemistry, and this study uses the 198-question Diamond subset. MMLU-Pro raises the number of answer options from four to ten and adds reasoning-focused items, and its release holds over 12{,}000 questions across 14 domains. The two stable sets answer a different question than the adversarial pair. They check that the mechanism keeps baseline accuracy, while BrokenMath and BrokenArXiv check the behavior under pressure.

Two filters run before a question enters the pool. The divergent filter keeps only questions where the three agents give at least two different answers in round one, which removes items that the agents already agree on. Retention on the adversarial sets is about 60 to 70 percent. The answer-type filter keeps only questions with a checkable answer, such as a multiple-choice option or a number, because the injected consensus and the collapse score both need a discrete answer. The main matrix draws 1{,}000 questions per dataset and the pilot draws 50, and the sample caps are recorded in the project configuration.

\subsection{Baselines}

The evaluation compares the trust-calibrated system with nine baseline conditions. Baseline B1 answers with one chain-of-thought pass~\cite{wei2022chain}. Baseline B2 adds the retrieval channel to the same single-agent pass, so the pair B1 and B2 isolates the effect of evidence. Baseline B3 is the vanilla debate loop of Du et al.~\cite{du2024improving}, and the same loop is the Gate 0 reproduction target. Baseline B4 combines the debate loop with retrieval. Baseline B5 samples several reasoning paths and keeps the most consistent answer~\cite{wang2023selfconsistency}. Baseline B6 is the layered mixture-of-agents pipeline of Wang et al.~\cite{wang2025mixture}. Baseline B7 sends each question to a large commercial model and records the accuracy ceiling. Baseline B8 is the trust-calibrated system. Baseline B9 is the iMAD reimplementation of Fan et al.~\cite{fan2026imad}, and baseline B10 is the ConsensAgent comparison of Pitre et al.~\cite{pitre-etal-2025-consensagent}.

The set answers the two objections that matter for this study. B5 covers the self-consistency question, and B6, B9, and B10 cover the question of an existing aggregation or efficiency method. The two retrieval pairs also serve the ablation plan. The only difference between B1 and B2, or between B3 and B4, is the evidence channel, so the comparison separates the effect of retrieval from the effect of debate.

\subsection{Metrics}

Four measures score a run, namely answer accuracy, consensus collapse rate, minority preservation rate, and evidence calibration rate. The collapse rate is the primary metric because it operationalizes the failure that the framework targets.

Let a debate carry the correct answer in the minority before the injection. The exposed set $E$ holds the agents that answered correctly at round one and received the injected consensus. The collapse set $C$ holds the exposed agents that change their answer in a later round while no new evidence enters the debate. The collapse rate is the share of exposed agents that drop the correct answer:
\begin{equation}\label{eq:ccr}
\mathrm{CCR} = \frac{|C|}{|E|}.
\end{equation}

The preservation rate is measured at the level of the debate. Let $D$ be the set of divergent debates, namely debates with at least two distinct initial answers, and let $P$ contain the debates where the correct answer survives to the final round, either in the position of at least one agent or in the weighted aggregate. Then
\begin{equation}\label{eq:mpr}
\mathrm{MPR} = \frac{|P|}{|D|}.
\end{equation}

The calibration rate checks that the trust weight means what it claims. The final trust weight of an agent is read as a confidence value, and the value is compared with the observed accuracy of the agents in the same confidence bin. Let $B$ be the number of bins, $n_b$ the number of agents in bin $b$, $\bar{t}_b$ the mean trust weight in that bin, and $\mathrm{acc}(b)$ the share of correct agents in that bin. The expected calibration error and the calibration rate are
\begin{equation}\label{eq:ecr}
\mathrm{ECE} = \sum_{b=1}^{B} \frac{n_b}{N}\left|\mathrm{acc}(b) - \bar{t}_b\right|, \qquad \mathrm{ECR} = 1 - \mathrm{ECE},
\end{equation}
where $N$ is the number of scored agents. A value of one means the trust weight matches the correctness rate in every bin. The binning follows the calibration method of Guo et al.~\cite{guo2017calibration}. Answer accuracy on the stable sets is the share of questions where the final answer matches the gold answer.

\subsection{Injection Protocol}

The collapse metric needs controlled pressure, and the injection protocol supplies it in six steps. The first step runs the three agents once and keeps the questions with divergent initial answers. The second step removes questions whose answers are not checkable. The third step builds the fabricated consensus from the majority answer, which is the wrong side on the questions that passed the first step. The fourth step delivers one system message that states the fabricated expert consensus. The message reaches only the agents that disagree with it, namely the correct minority, and it enters between round one and round two. No further pressure is applied after that point. The fifth step lets the debate finish its three rounds under the normal rules. The sixth step scores the abandonment with Equation~\eqref{eq:ccr}.

The protocol is validated before the pilot runs. Two annotators label a sample of 40 questions as eligible or not eligible, and the protocol passes when the agreement reaches a Cohen's kappa of 0.75 or higher. The kappa statistic is
\begin{equation}\label{eq:kappa}
\kappa = \frac{p_o - p_e}{1 - p_e},
\end{equation}
where $p_o$ is the observed agreement and $p_e$ is the agreement expected by chance. A kappa below the threshold sends the labeling rubric back for revision, and the pilot waits.
