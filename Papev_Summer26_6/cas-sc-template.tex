...
The evaluation uses five public scientific question sets in three roles: two adversarial sets that create a false majority, one ceiling set of expert-level questions, and two stable sets for accuracy comparison.

The adversarial pair is BrokenMath and BrokenArXiv. BrokenMath is built from 2025 competition problems that a language model perturbs into false statements, and human experts then review the perturbed versions. The release contains 451 proof problems under CC BY-NC-SA 4.0~\cite{petrov2025brokenmath}. BrokenArXiv turns false statements from recent arXiv papers into problems and publishes new snapshots each month. The 2026 release used here contains 87 problems under CC BY-SA 4.0~\cite{dekoninck2026matharena}. Both sets place an agent in front of a wrong premise that reads as plausible, which is the condition that drives sycophantic agreement in a debate.

Humanity's Last Exam sets the ceiling with 2{,}500 expert-written questions across dozens of subject areas, released under MIT terms behind a click-through agreement and a no-redistribution rule~\cite{phan2026hle}. GPQA contains 448 graduate-level multiple-choice questions in biology, physics, and chemistry under CC BY 4.0, and this study uses the 198-question Diamond subset~\cite{rein2024gpqa}. MMLU-Pro raises the answer options from four to ten and adds reasoning-focused items, and its MIT release holds over 12{,}000 questions across 14 domains~\cite{wang2024mmlupro}. The stable sets check that the mechanism keeps baseline accuracy, while the adversarial sets check the behavior under pressure.

Two filters run before a question enters the pool. The divergent filter keeps only questions where the three agents give at least two different answers in round one, with about 60 to 70 percent retention on the adversarial sets. The answer-type filter keeps only questions with a checkable answer, such as a multiple-choice option or a number. The main matrix draws 1{,}000 questions per dataset and the pilot draws 50.
