...
|-------|---------|---------|---------|-----|-----------|
| **Dev** (Ph 0–2) | [Qwen3.5-9B](https://huggingface.co/Qwen/Qwen3.5-9B) | [Gemma 4 12B](https://huggingface.co/google/gemma-4-12B) | [Ministral-3-14B-Instruct-2512](https://huggingface.co/mistralai/Ministral-3-14B-Instruct-2512) | RTX A6000 48GB ($0.53/hr) | ~$160–320 |
| **Final** (Ph 3–5) | [Qwen3.6-27B](https://huggingface.co/Qwen/Qwen3.6-27B) | [Gemma 4 26B A4B](https://huggingface.co/google/gemma-4-26B-A4B) | [Mistral-Small-3.2-24B-Instruct-2506](https://huggingface.co/mistralai/Mistral-Small-3.2-24B-Instruct-2506) | A100 80GB ($0.68–1.50/hr) | ~$400–800 |

During development, all pipeline code (claim decomposition, retrieval, trust update, LangGraph state machine, injection protocol) is written and debugged using the smaller Dev models (at Q4/FP8 quantisation, ~28GB total) on a cheap RTX A6000 48GB. The final experiment matrix is then run once on the full-scale models using an A100 — no code changes needed, only a model-name swap in the config.
...
| ------------------- | -------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------- |
| **Agent 1**         | [Qwen3.5-9B](https://huggingface.co/Qwen/Qwen3.5-9B)                 | [Qwen3.6-27B](https://huggingface.co/Qwen/Qwen3.6-27B)                 | Dev: cheap iterative testing. Final: 27B dense, caps 397B MoE predecessor in coding, 262K context, Apache 2.0                  | Q4 ~17 GB (final); validated on dev models before A100 run       |
...
| **Inference**      | vLLM                           | vLLM                           | Free, fast, standard for local multi-model serving; same engine for both Dev (A6000) and Final (A100)                           | Setup complexity on first use only                                                                                                |
...
|------|-------------|----------------|-------|
| **GPU** | RTX A6000 48GB | A100 80GB | Ampere workstation vs datacenter |
...
| **Implementation** | Single A100 compute ceiling under full 9-baseline × multi-dataset × 3-seed matrix                                           | Medium                                       | Moderate                             | Two-phase GPU strategy: Dev on RTX A6000 48GB ($0.53/hr), Final on A100 ($0.68–1.50/hr). Core replication ~250–500 A100-hrs; total GPU budget ~$560–1,100                                                                                                                |
