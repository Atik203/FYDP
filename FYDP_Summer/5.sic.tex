...
\begin{center}
    \begin{table}[ht]
        ...
        \begin{tabular}{|p{0.32\textwidth}|p{0.18\textwidth}|p{0.18\textwidth}|p{0.18\textwidth}|}
            ...
            \hline
            RTX PRO 6000 Blackwell 96GB rental           & 300 hours & 1.10--2.70 / hour & 330--810          \\
            \hline
...
\begin{center}
    \begin{table}[ht]
        \centering
        \caption{GPU hours and cost split by phase (RTX PRO 6000, USD 1.10--2.70 per hour).}
        \renewcommand{\arraystretch}{1.6}
        \begin{tabular}{|p{0.32\textwidth}|p{0.18\textwidth}|p{0.18\textwidth}|}
            \hline
            Phase                                  & Hours        & Cost (USD)        \\
            \hline
            Phase-0 and Phase-1 (setup, baselines) & 30           & 33--81            \\
            \hline
            Phase-2 (trust build)                  & 60           & 66--162           \\
            \hline
            Phase-3a and Phase-3b (main runs)      & 140          & 154--378          \\
            \hline
            Phase-4 (human eval)                   & 40           & 44--108           \\
            \hline
            Phase-5 (writing, final checks)        & 30           & 33--81            \\
            \hline
            \textbf{Total}                         & \textbf{300} & \textbf{330--810} \\
            \hline
        \end{tabular}
...
Costs for storage are estimated as \$10.00 based upon an initial cost of \$0.05/GB and a total of 200 GB. Logs and result package models will be approximately 180 GB in size while the remaining space will be used for the workspace. The direct costs of the retrieval API services from PubMed, arXiv, Semantic Scholar and OpenAlex are free due to their use of free tier service levels; however, the limits imposed may increase the time required to retrieve the data. As both GPQA~\cite{rein2024gpqa} and MMLU-Pro~\cite{wang2024mmlupro} datasets are publicly available, there were no additional costs associated with these datasets. Costs associated with printing (approximately \$15) and Internet connectivity were assumed to be fixed, therefore included in the overall estimate. Overleaf was provided to the team, thus eliminating any costs for this platform.

Additionally, an alternative budget was developed. Rented usage of an A100 80GB is currently about \$1.78 per hour, or about \$534 over the course of 300 hours. The A100 is no longer the cheaper option, and it also offers less memory, namely 80 GB against 96 GB, and a weaker FP8 path than the RTX PRO 6000. The RTX PRO 6000 was chosen because the Blackwell FP8 path keeps the pipeline simple and avoids the engineering risk of tighter quantization or sequential model loading.
...
\begin{center}
    \begin{table}[ht]
        ...
        \begin{tabular}{|p{0.32\textwidth}|p{0.18\textwidth}|p{0.18\textwidth}|p{0.18\textwidth}|}
            ...
            Item             & Quantity  & Unit Cost (USD)   & Total (USD)       \\
            \hline
            A100 80GB rental & 300 hours & 1.78 / hour & 534               \\
            \hline
...
A ten percent buffer (approximately \$34-\$82) is maintained to cover unexpected price increases and extended operating time in the event of a failed run. The buffer can be used to fund up to thirty additional hours of operation if either an ablation run fails or a maintenance call is required on a gate before the end of the plan period.
...
\begin{center}
    \begin{table}[ht]
        ...
        \begin{tabular}{|p{0.28\textwidth}|p{0.14\textwidth}|p{0.44\textwidth}|}
            ...
            \hline
            K1: Natural sciences         & $\sqrt{}$ & Scientific questions; evidence from PubMed, arXiv, Semantic Scholar, OpenAlex \\
            \hline
...
\paragraph{K1: Natural sciences} K1 has been addressed. The framework is designed to address scientific inquiries and verify each claim by comparing it to articles on PubMed, arXiv, Semantic Scholar, and OpenAlex. Natural science questions are included in evaluation data sets. The project itself is not an outcome of a new natural science theory. The problem domain and the evidence base are still sourced from the natural science fields. This is the reason for marking K1 as covered.
...
Table~\ref{tab:po_map} maps FYDP I against the twelve program outcomes.

\begin{center}
    \begin{table}[ht]
        \centering
        \caption{Mapping with program outcomes (PO1--PO12).}
        \renewcommand{\arraystretch}{1.6}
        \begin{tabular}{|p{0.13\textwidth}|p{0.13\textwidth}|p{0.13\textwidth}|p{0.13\textwidth}|p{0.13\textwidth}|p{0.13\textwidth}|}
            \hline
            PO1                            & PO2              & PO3                             & PO4           & PO5                            & PO6                      \\
            Engineering knowledge          & Problem analysis & Design/development of solutions & Investigation & Modern tool usage              & The engineer and society \\
            \hline
            $\sqrt{}$                      & $\sqrt{}$        & $\times$                        & $\sqrt{}$     & $\times$                       & $\times$                 \\
            \hline
            PO7                            & PO8              & PO9                             & PO10          & PO11                           & PO12                     \\
            Environment and sustainability & Ethics           & Individual and teamwork         & Communication & Project management and finance & Life-long learning       \\
            \hline
            $\times$                       & $\times$         & $\times$                        & $\sqrt{}$     & $\sqrt{}$                      & $\sqrt{}$                \\
            \hline
        \end{tabular}
        \label{tab:po_map}
    \end{table}
...
The real-life problem in Chapter 1 addresses six program outcomes (PO), all of which are addressed in FYDP I. Literature review and Gap analysis (Chapter 2) will cover PO4. PO2 is covered by the requirements work in Chapter 3. PO10 is addressed in the interim report and in the oral presentation. The timeline is applicable to PO11 and the budget is applicable to PO11. The writing of the journal and the independent learning record are used to cover PO12. The other six program outcomes will be addressed in FYDP II and FYDP III.

\section{Summary}
The chapter defined the requirements for the software-layer, hardware-layer, and communication-layers and documented the economic, environmental, ethics-social constraints that have impacted design decisions. Budget-analysis demonstrates that the project can fit within a students' budget utilizing a rented RTX PRO 6000 Blackwell GPU with a phased split of costs over the 300 hours plus a 10\% buffer. The complex engineering mapping demonstrates that six out of seven P attributes are covered with only P6 being non-covered due to little to no input from external stakeholders. For knowledge, K1-K8 are all covered. For activities, A1-A2 and A5 are covered while A3 and A4 are not. For the program outcomes, FYDP I covers PO1, PO2, PO4, PO10, PO11 and PO12.
